...
function StandardData() {
  ...
  const fetchStandardData = async () => {
    try {
      ...
      const response = await fetch('/api/admin/standard-data', {
        headers: {
  ...
  const handleDeleteAllData = async () => {
    if (window.confirm('确定要删除所有国标数据吗？此操作不可恢复。')) {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('/api/admin/standard-data/clear', {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handleExportDataXlsx = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/admin/standard-data/export/xlsx', {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleFileImportXlsx = async (e) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/standard-data/import/xlsx', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  const importData = async (data) => {
    try {
      ...
      // 清空现有数据
      await fetch('/api/admin/standard-data/clear', {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      for (const item of data) {
        await fetch('/api/admin/standard-data', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,

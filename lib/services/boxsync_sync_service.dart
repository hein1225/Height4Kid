...
class BoxSyncSyncService implements SyncService {
  ...
  Future<bool> _createAppPartition() async {
    try {
      print('BoxSync - Creating app partition for Height4Kid');
      final response = await http.post(
      ...
      print('BoxSync createApp - Response status: ${response.statusCode}');
      print('BoxSync createApp - Response body: ${response.body}');

      if (response.statusCode == 200 || response.statusCode == 201) {
        final data = jsonDecode(response.body);
        // 200 表示创建成功，409 表示应用已存在
        if (data['success'] == true || response.statusCode == 409) {
          print('BoxSync - App partition ready');
          return true;
        }
      } else if (response.statusCode == 409) {
        // 应用分区已存在，也算成功
        print('BoxSync - App partition already exists');
        return true;
      }
      return false;
    } catch (e) {
      print('BoxSync createApp - Error: $e');
  ...
  Future<SyncResult> uploadData(String data, String filename) async {
    ...
    try {
      ...
      print('BoxSync upload - URL: $_baseUrl/api/sync/write');
      print('BoxSync upload - Filename: $filename');
      print('BoxSync upload - Data size: ${data.length} bytes');
      print('BoxSync upload - Request body: $requestBody');
      ...
      print('BoxSync upload - Response status: ${response.statusCode}');
      print('BoxSync upload - Response body: ${response.body}');
  ...
  Future<List<CloudFileInfo>> listFiles(String directory) async {
    ...
    try {
      ...
      print('BoxSync listFiles - URL: $url');
      ...
      print('BoxSync listFiles - Response status: ${response.statusCode}');
      print('BoxSync listFiles - Response body: ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          print('BoxSync listFiles - Found ${result.length} files');
          return result;
        } else {
          print('BoxSync listFiles - Server returned success: false');
        }
      } else {
        print('BoxSync listFiles - HTTP error: ${response.statusCode}');
      }
      return [];
    } catch (e) {
      print('BoxSync listFiles - Error: $e');
      return [];
    }
  }

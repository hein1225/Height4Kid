...
class NextcloudConfigScreen extends StatefulWidget {
  const NextcloudConfigScreen({super.key});

  @override
  State<NextcloudConfigScreen> createState() => _NextcloudConfigScreenState();
}

class _NextcloudConfigScreenState extends State<NextcloudConfigScreen> {
  final _formKey = GlobalKey<FormState>();
  final _serverUrlController = TextEditingController();
  final _usernameController = TextEditingController();
  final _passwordController = TextEditingController();
  final _remotePathController = TextEditingController(text: '/Apps/Height4Kid/');
  ...
  Future<void> _testConnection() async {
    ...
    final result = await appProvider.testServiceConnection(SyncServiceType.nextcloud);
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                            color: const Color(0xFF0082C9).withValues(alpha: 0.3),
                          ),
                        ),
                        child: const Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            SizedBox(height: 8),
                            Text(
                              'Nextcloud 是基于 WebDAV 协议的自托管云存储解决方案。建议使用应用专用密码进行连接。',
                              style: TextStyle(
                                fontSize: 14,
  ...
  Future<void> _manualSync(bool upload) async {
    ...
    if (upload) {
      final result = await appProvider.manualUploadToService(SyncServiceType.nextcloud);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(result.message),
  ...
  Future<void> _showVersionSelectionDialog(AppProvider appProvider) async {
    // 获取云端版本列表
    final versions = await appProvider.getCloudVersions(SyncServiceType.nextcloud);
